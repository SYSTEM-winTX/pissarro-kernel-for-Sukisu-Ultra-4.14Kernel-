# Build Pissarro Kernel with SukiSU Ultra (4.14)

一键云端编译 **Build Pissarro Kernel with SukiSU Ultra (4.14)**（`achilles6_row_wifi`）内核并集成 **SukiSU Ultra**，自动打包为 **AnyKernel3** 可刷入包。
...
4. 完成后到 **Actions → Artifacts** 下载 `压缩包` 刷入设备。
...
# 拉仓库
git clone [https://github.com/yourname/x606f-sukisu-builder.git](https://github.com/SYSTEM-winTX/0.git)
cd pissarro-kernel-for-Sukusu-Ultra-4.14Kernel-
...
## 默认设置
- 内核源码：`[https://github.com/MatiDEV-PL/kernel_lenovo_achilles6_row_wifi](https://github.com/SYSTEM-winTX/0)`（可通过环境变量自定义）
